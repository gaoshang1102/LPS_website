...
datDay2_2 <- cbind(h5read("./Day2_hdf5file.h5", name = "Dimension_reduction/UMAP/df_2D"), 
                   h5read("./Day2_hdf5file.h5", name = "Dimension_reduction/TSNE/df_2D"))
expressions <- h5read("./Day2_hdf5file.h5", name = "Normalized_Data/Normalized_Data_matrix")
rownames(expressions) <- h5read("./Day2_hdf5file.h5", name = "Gene_meta/Gene_Names")
colnames(datDay2_2)[1:2] <- c("UMAP_1","UMAP_2")
p_2d_plot <- function(rd_method, gene_name){
  plot_2d <- ggplot(datDay2_2, aes(datDay2_2[, paste0(rd_method, "_1")], datDay2_2[, paste0(rd_method, "_2")], 
