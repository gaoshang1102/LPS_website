...
colnames(datDay0_1)[1:2] <- c("UMAP_1","UMAP_2")
p_2d_plot <- function(rd_method, cluster_no){
  plot_2d <- ggplot(datDay0_1, aes(datDay0_1[, paste0(rd_method, "_1")], datDay0_1[, paste0(rd_method, "_2")], 
                                   color = factor(ifelse(datDay0_1$cluster==as.integer(cluster_no), as.integer(cluster_no), 0)))) +
    ...
    scale_color_manual(name = paste0("cluster ", cluster_no), 
                       values = c("#F1F1F1", gg_color_hue(length(table(datDay0_1$cluster)))[as.integer(cluster_no)] ))
  return(plot_2d)
}
